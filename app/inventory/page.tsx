...
// These MUST live outside InventoryPage. Defining a component function
// inside another component's body means React creates a brand-new function
// reference for it on every render — React treats that as a totally
// different component, so it unmounts the old DOM and mounts fresh DOM
// every time. For anything with a focused <input> inside (like the search
// box below), that means focus gets dropped after every single keystroke.
// Hoisting them here, taking their data via props instead of closure,
// gives them a stable identity across renders so React reuses the DOM
...
function CashierTopNav({ onLogout }: { onLogout: () => void }) {
  const links = [
    { href: '/pos', label: 'POS' },
...
function ProductionTopNav({ onLogout }: { onLogout: () => void }) {
  return (
    <div className="relative z-10 w-full flex items-center gap-6 px-6 py-3 shrink-0" style={{ backgroundColor: '#7B1111' }}>
      <div className="flex items-center gap-3 shrink-0">
        <span className="text-white font-black text-xl tracking-wide">IS FREDS</span>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-sm w-full max-w-md" style={{ boxShadow: '4px 4px 20px rgba(0,0,0,0.4)' }}>
        <div className="px-6 py-4" style={{ backgroundColor: type === 'pullout' ? '#991B1B' : '#5B21B6' }}>
          <h2 className="text-white font-black text-lg">{type === 'pullout' ? 'Pull-out Stock' : 'On the House'}</h2>
        ...
        <form onSubmit={onSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input type="number" min="1" value={quantity} onChange={e => setQuantity(e.target.value)} required placeholder="e.g., 3" className={inputClass} />
          </div>
          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={submitting}
              className="flex-1 py-2 rounded-sm font-bold text-white text-sm disabled:opacity-50"
              style={{ backgroundColor: type === 'pullout' ? '#991B1B' : '#5B21B6' }}>
              {submitting ? 'Recording...' : `Confirm ${type === 'pullout' ? 'Pull-out' : 'OTH'}`}
            </button>
...
export default function InventoryPage() {
  ...
  if (userRole === 'cashier') {
    ...
    return (
      <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#F5A623' }}>
        <CashierTopNav onLogout={handleLogout} />
        ...
        <DisposalModal
          show={showDisposalModal} product={disposalProduct} type={disposalType} location={disposalLocation}
  ...
  if (userRole === 'production') {
    ...
    return (
      <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#F5A623' }}>
        <ProductionTopNav onLogout={handleLogout} />
        <div className="flex flex-1 relative">
          <LogoWatermark />
          <div className="relative z-10 flex-1 p-6 overflow-y-auto">
